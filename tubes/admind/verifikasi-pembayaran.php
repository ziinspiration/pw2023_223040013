...
$brg = query("SELECT p.*, kb.*, b.*, u.*, p.id as id_pembayaran
              ...
              WHERE p.id AND p.status = 'Belum di konfirmasi'");

if (isset($_POST['setuju'])) {
    $id = $_POST['id'];
    $query = "UPDATE pembayaran SET status = 'Sukses' WHERE id = $id";
    if (mysqli_query($conn, $query)) {
        echo "
            <script>
                alert('Konfirmasi persetujuan berhasil!');
                window.location.href = 'verifikasi-pembayaran.php';
            </script>
        ";
    } else {
        echo "
    ...
    <?php foreach ($brg as $b) : ?>
        <form action="" method="post">
            <div class="card card-pesanan ps-3">
                ...
                <!--  -->
                <a class="mb-3 text-decoration-none rincian" href="rincian-pembayaran.php?id=<?= $b["id_pembayaran"] ?>">Rincian pembayaran</a>
                <!--  -->
                <input type="hidden" name="id" value="<?= $b['id_pembayaran']; ?>">
                <button name="setuju" class="btn btn-setujui btn-primary mb-2">Setujui</button>
            </div>
        </form>
    <?php endforeach; ?>
